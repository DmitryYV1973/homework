...
for _ in range(min(user_input, len(proverbs), len(variants))):  # Ограничиваем количество итераций
    random_proverb = proverbs.pop(random.randrange(len(proverbs)))  # Выбор и удаление пословицы
    random_variant = variants.pop(random.randrange(len(variants)))  # Выбор и удаление существительного
    modified_proverb = random_proverb.replace('Ум', random_variant)  # Замена "Ум" на выбранное существительное
    result.append(modified_proverb) 
